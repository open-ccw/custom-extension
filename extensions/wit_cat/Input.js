...
class WitCatInput {
  ...
  createinput(args) {
    ...
    sstyle.fontSize = `${
      this.adaptive ? (parseFloat(this.canvas().style.width) / 360) * Number(args.size) : Number(args.size)
    }px`;
    sstyle.resize = "none";
      ...
      if (!this.adaptive) {
        ...
        const callback = () => {
          ...
          Array.prototype.map.call(search, (item) => {
            ...
            if (fontsize) {
              item.style.fontSize = `${(parseFloat(this.canvas().offsetWidth) / 360) * fontsize}px`;
            }
          });
        };
  ...
  _addevent() {
    ...
    document.addEventListener("keydown", (event) => {
      ...
      if (
        ...
        event.target.id.startsWith("WitCatInput")
      ) {
        if (this.InputListen.find((e) => e.id === event.target.id.split("WitCatInput")[1]).input) {
          if (
            this.keypresss(
              this.keypresslist,
              this.InputListen.find((e) => e.id === event.target.id.split("WitCatInput")[1]).more.split(",")
            )
          ) {
          ...
          if (
            this.keypresss(
              this.keypresslist,
              this.InputListen.find((e) => e.id === event.target.id.split("WitCatInput")[1]).enter.split(",")
            )
          ) {
