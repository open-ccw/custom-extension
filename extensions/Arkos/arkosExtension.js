...
class ArkosExtensions {
  constructor(runtime) {
    ...
    this._formatMessage = runtime.getFormatMessage({
      ...
      en: {
        ...
        "ArkosExt.addToSortedTable":
          "📊add (overwrite if existed)[name]to table[list] with sort index value[value],extra data[extra] and sort",
  ...
  turnDegreesToDir(args, util) {
    ...
    if (Math.abs(dif) < degree) util.target.setDirection(dir);
    else if (dif < 0) util.target.setDirection(util.target.direction - degree);
    else util.target.setDirection(util.target.direction + degree);
  }
  ...
  _findPlaceAndInsert(list, order, item) {
    ...
    for (let i = 0; i < list.length; i++) {
      if (
        (list[i].rankValue > item.rankValue && order === "asc") ||
  ...
  addToSortedTable(args) {
    this.createTableIfNotExist(args.list);
    this._findPlaceAndInsert(this.sortedTable[args.list].list, this.sortedTable[args.list].order, {
      name: args.name,
  ...
  addTempVar(args) {
    this.tempData[Cast.toString(args.var)] =
      Cast.toNumber(this.tempData[Cast.toString(args.var)]) + Cast.toNumber(args.t);
  }
