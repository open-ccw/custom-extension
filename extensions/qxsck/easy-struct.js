...
class easyStruct {
  constructor(runtime) {
    ...
    this._formatMessage = runtime.getFormatMessage({
      en: {
        ...
        "qxsckeasystruct.sort.tip":
          'Please ensure that the sorting rule is one of ascending is "asc",\
        ...
        "qxsckeasystruct.setStructListMemberData":
          "set item [INDEX] of member [MEMBER] in struct list [NAME] to [DATA]",
      ...
      "zh-cn": {
        ...
        "qxsckeasystruct.sort.tip":
          "请确保排序规则为升序“asc”，降序“desc”，不作排序“none”，\
        ...
        "qxsckeasystruct.setStructListMemberData":
          "设置结构体列表 [NAME] 的第 [INDEX] 项的成员 [MEMBER] 的数据为 [DATA]",
          ...
          if (value !== "none") {
            ...
            if (type_ === "str") {
              let aval = a[key],
                bval = b[key];
              cmp_ =
                value === "asc" ? (aval < bval ? -1 : aval > bval ? 1 : 0) : aval > bval ? -1 : aval < bval ? 1 : 0;
            } else if (type_ === "num") {
              let ak = Number(a[key]),
                bk = Number(b[key]);
  ...
  setStructType(args) {
    let name = String(args.NAME),
      members = String(args.MEMBERS).split(" ");
  ...
  setStructMemberData(args) {
    ...
    if (name in this.useStructs)
      if (member in this.useStructs[name]["data"]) this.useStructs[name]["data"][member] = data;
  }
  ...
  structListPushData(args) {
    let name = String(args.NAME),
      num = Number(args.NUM);
    if (name in this.useStructLists) {
      let type = this.useStructLists[name]["type"],
        members = this.structs[type],
        json = {};
  ...
  setStructListMemberData(args) {
    ...
    if (name in this.useStructLists)
      if (index <= this.useStructLists[name]["data"].length)
        if (member in this.useStructLists[name]["data"][index - 1])
          this.useStructLists[name]["data"][index - 1][member] = data;
  }
  getStructListMember(args) {
    ...
    if (name in this.useStructLists) {
      if (index <= this.useStructLists[name]["data"].length) {
        if (member in this.useStructLists[name]["data"][index - 1])
          return this.useStructLists[name]["data"][index - 1][member];
        else return "";
      } else return "";
    } else return "";
  }
  ...
  deleteStructListMember(args) {
    let name = String(args.NAME),
      index = Number(args.INDEX);
    if (name in this.useStructLists)
      if (index <= this.useStructLists[name]["data"].length) this.useStructLists[name]["data"].splice(index - 1, 1);
  }
  sortHelp() {
  ...
  structListSort(args) {
    let name = String(args.NAME),
      rule_arr = String(args.RULE).split(" ");
    if (name in this.useStructLists) {
      let type = this.useStructLists[name]["type"],
        members = this.structs[type];
      let flag = 0;
      if (members.length === rule_arr.length / 3 && rule_arr.length % 3 === 0) {
        ...
        for (let i = 0; i < rule_arr.length / 3; i++) {
          if (
            (rule_arr[i * 3 + 2] !== "asc" && rule_arr[i * 3 + 2] !== "desc" && rule_arr[i * 3 + 2] !== "none") ||
            (rule_arr[i * 3 + 1] !== "num" && rule_arr[i * 3 + 1] !== "str")
          ) {
            flag = i;
            break;
          } else
            (rule_member.push(rule_arr[i * 3]),
              rule_type.push(rule_arr[i * 3 + 1]),
              rule_order.push(rule_arr[i * 3 + 2]));
        }
        if (flag === 0) {
          ...
          console.log(
            'Please ensure that the sorting rule is one of ascending is "asc",descending is "desc" or not sorting is "none",'
          );
          console.log(
            'the sorting type is one of string "str" for lexicographic sorting or number "num" for size sorting.'
          );
        }
      }
    }
